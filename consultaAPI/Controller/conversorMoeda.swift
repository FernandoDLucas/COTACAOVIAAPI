...
func conversorMoeda(){
    ...
    var qtdMoeda : String? = nil
    while (qtdMoeda == nil){
        print("Insira a quantidade da moeda origem: ", terminator: "")
        
        if let qtdInput = readLine(){
            let num = Float (qtdInput)!
            let num2 = Float (result.response.exchangeRate)!
            print("O valor de \(qtdInput) \(moedaOrigem!) atualmente vale \(num * num2) \(moedaDestino!)")
            qtdMoeda = qtdInput
        } else {
